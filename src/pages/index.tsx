...
const workflow = [
  {
    step: "01",
    title: "Drop in raw JSON",
    detail: "Paste any payload and auto-expand nested objects.",
  },
  {
    step: "02",
    title: "Link reusable blocks",
    detail: "Connect references visually instead of duplicating data.",
  },
  {
    step: "03",
    title: "Export final model",
    detail: "Download the resolved structure when it is ready to ship.",
  },
];
    ...
    <main className="mt-[1.4rem] grid gap-[1.05rem]">
      <section className="grid grid-cols-1 gap-0">
        <div className="animate-rise rounded-[32px] p-[clamp(1.35rem,3vw,2.65rem)] shadow-[0_24px_50px_rgba(70,28,47,0.2)] [background:radial-gradient(circle_at_14%_14%,rgba(255,255,255,0.52),transparent_44%),radial-gradient(circle_at_78%_82%,rgba(255,237,172,0.26),transparent_42%),linear-gradient(135deg,#ffe0be_0%,#ff8e85_44%,#de4d4f_100%)]">
          ...
          <p className="m-0 max-w-[52ch] text-[1.04rem] leading-[1.48] text-[#412129]">
            ...
            can structure data faster, review changes clearly, and avoid schema
            drift.
          </p>
          <div
          ...
          >
            {metrics.map((metric) => (
              <div
                ...
                key={metric.label}
              >
                <p className="m-0 text-[1.1rem] font-bold text-[#2a1018]">
                  {metric.value}
                </p>
          ...
          <div className="mt-[1.1rem] flex flex-wrap gap-[0.65rem]">
            {/* @ts-expect-error - TanStack Router Link type inference issue */}
            <Link
              to="/workspace"
              viewTransition
      ...
      >
        <h2 className="m-0 text-[1.2rem]">
          From raw payload to production-ready model
        </h2>
